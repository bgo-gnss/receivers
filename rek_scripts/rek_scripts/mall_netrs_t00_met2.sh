...
# HG aug 2009: some changes made for kidj automatic download
######################
RBINDIR=/home/gpsops/bin/5700_v255/
...
#   ${TEQCPATH}/teqc_2006 -week ${WEEK} -tr d $DATFIL > tmp1.$$
#   ${TEQCPATH}/teqc_2006 +err err.lst -config ${CONPATH}/${CONFIL} tmp1.$$ >$OBSFIL
   #${TEQCPATH}/teqc -week ${WEEK} -tr d $DATFIL > tmp1.$$
   ${TEQCPATH}/teqc -tr d $DATFIL > tmp1.$$
   ${TEQCPATH}/teqc +err err.lst -config ${CONPATH}/${CONFIL} tmp1.$$ >$OBSFIL
   #/home/gpsops/bin/thjap $OBSFIL

   # move and clean up
   rm ${RELBAS}.dat ${RELBAS}.ion ${RELBAS}.mes ${RELBAS}.eph tmp1.$$ err.lst
