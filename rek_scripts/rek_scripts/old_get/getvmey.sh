...
# Variables:
STATION_FULL_NAME="Vestmannaeyjar"
STATION=VMEY
station=vmey

IP=157.157.222.54       		# netr9 web access and programable interface
HTTP_PORT=':8060'			# HTTP
FTP_PORT=':2160'			# FTP
...
DIRLIST=`curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/' 2>/dev/null|  grep 'directory name' | grep -v 'lost' | awk '{print substr($2,6,6)}' | grep -v 'Clone' | tail -${MONTHS}`
