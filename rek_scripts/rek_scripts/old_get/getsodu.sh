#!/bin/sh
# Hourly and daily download of Trimble NETRS receiver SODU
# SFS sept 2011: Adapted from getgfum.sh to download from SODU.
#################
...
STATION=SODU
STATION_FULL_NAME="Söðulfell"
station=sodu
sss=sodu
USER=gps
PASSWORD=katla1918

IPNR=130.208.196.45     		# netrs ipnumber 
IPNRFTP=130.208.196.45:8088    		# netrs ipnumber
...
echo "#######################################################################"
echo " "
echo "This is a download script for $STATION_FULL_NAME ($STATION) IP:$IPNR"
echo `date`
echo " "
...
TSTR=`curl -u $USER:$PASSWORD 'http://'$IPNRFTP'/prog/show?temperature'`
echo ">> "$TSTR
VSTR=`curl -u $USER:$PASSWORD 'http://'$IPNRFTP'/prog/show?Voltage&input=2'`
echo ">> "$VSTR
...
DIRLIST=`curl -u $USER:$PASSWORD 'http://'${IPNRFTP}'/prog/show?loggedfiles&directory=/' 2>/dev/null|  grep Directory | awk '{print substr($2,6,6)}' | tail -2`
for MDIR in $DIRLIST
do
  cd ${LSDIR}
...
######## get a-file list:  #########
  ...
  curl -u $USER:$PASSWORD 'http://'${IPNRFTP}'/prog/show?loggedfiles&directory=/'${MDIR}'/a'  2>/dev/null | grep name | awk '{print substr($2,6,22), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
   ...
   # wget -c ftp://${IPNRFTP}/$MDIR/a/$d
    curl --user $USER:$PASSWORD 'http://'${IPNRFTP}'/prog/download?loggedfile&path=/'$MDIR'/a/'$d'' > $MDIR/a/$d
...
######## get b-file list:  #########
  ...
  curl --user $USER:$PASSWORD 'http://'${IPNRFTP}'/prog/show?loggedfiles&directory=/'${MDIR}'/b'  2>/dev/null | grep name | awk '{print substr($2,6,22), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  ...
  do
    ...
    curl --user $USER:$PASSWORD 'http://'${IPNRFTP}'/prog/download?loggedfile&path=/'$MDIR'/b/'$d'' > $MDIR/b/$d
  done            # closes file loop
done            # closes month loop
