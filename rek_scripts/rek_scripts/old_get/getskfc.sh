...
# Variables:
STATION_FULL_NAME="Skaftafell"
STATION=SKFC
station=skfc

IP=157.157.222.213          # Leica web access and programable interface
HTTP_PORT=':8060'			# HTTP
FTP_PORT=':2160'			# FTP

LSDIR=/home/gpsops/tmp/SKFC_data          #CREATE# 
LOGDIR=/home/gpsops/log/getdata/$STATION  #CREATE# 
TMPDIR=/$LSDIR/tmp	#CREATE# A-files
TMPDIR1=/$LSDIR/tmp1   #CREATE# B-files
...
DIRLIST=`curl 'ftp://'$IP$FTP_PORT'/SD\ Card/Data/15s_24hr' 2>/dev/null |  awk '{print substr($2,6,6)}' | grep -v '30s_8h' | tail -${MONTHS}`
