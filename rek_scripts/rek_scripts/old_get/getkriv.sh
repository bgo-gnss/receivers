#!/bin/sh
# Hourly and daily download of Trimble NETRS receiver KRIV
# SFS jun 2011: Adapted from getsoho.sh to download from KRIV which now has 3G conection.
#################
...
STATION=KRIV
STATION_FULL_NAME="Krísuvík"
station=kriv
sss=kriv

#IPNR=194.144.255.177:8080      	# netrs ipnumber 
#IPNRFTP=194.144.255.177    		# netrs ipnumber
#IPNR=157.157.40.222:8080      		# netrs ipnumber 
#IPNRFTP=157.157.40.222    		# netrs ipnumber
IPNR=157.157.124.129:8080               # netrs ipnumber
IPNRFTP=157.157.124.129                 # netrs ipnumber
...
PNGCNT=`ping -c1 -w1 $IPNRFTP |grep transmit|awk '{print $4}'`
  ...
  then
    ...
    echo ">> Will try again"
    sleep 3
    PNGCNT=`ping -c1 -w4 $IPNRFTP |grep transmit|awk '{print $4}'`
    ...
    then
       ...
       echo ">> Will try again"
       sleep 3
       PNGCNT=`ping -c1 -w8 $IPNRFTP |grep transmit|awk '{print $4}'`
...
TSTR=`curl 'http://'$IPNR'/prog/show?temperature' > /dev/null 2>&1` 
echo ">> "$TSTR
VSTR=`curl 'http://'$IPNR'/prog/show?Voltage&input=2' > /dev/null 2>&1`
echo ">> "$VSTR
