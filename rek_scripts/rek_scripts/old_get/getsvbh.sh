#!/bin/sh
# Hourly and daily download of Trimble NETRS receiver SVBH
#HG
# HG dec 2009: Use dump instead of brot
# HG jun 2010: Use wget download line from Benni Ofeigs. Note that SVBH does not sample into session sub-directory
#################
STATION=SVBH
station=svbh
sss=svbh
...
# Variables:
IPNR=157.157.145.99:8080       # netrs ipnumber
LSDIR=/data/$STATION
...
do
  ...
  curl 'http://'${IPNR}'/prog/show?loggedfiles&directory=/'${MDIR}''  2>/dev/null | grep "LoggedFile name" | awk '{print substr($2,6,22), substr($3,6,8)}' > tmplist.tmp
  ...
  echo "there are `grep SVBH dlist.tmp | wc -l | awk '{print $1}'` 15 sec files to download from the receiver for month $MDIR"
  DLIST=`cat dlist.tmp`
  for d in $DLIST
  do
    echo "getting file $d"
    ...
    cd $LSDIR/$MDIR
    #wget -c -a /tmp/SVBH-downlog ftp://157.157.145.99/$MDIR/$d
    wget -c ftp://157.157.145.99/$MDIR/$d
...
#  # get missing b-files:
#  echo "there are `grep SVBH dlist.tmp | wc -l | awk '{print $1}'` 1 sec files to download from the receiver for month $MDIR"
#  DLIST=`cat dlist.tmp`
#  for d in $DLIST
#  do
#    echo "getting file $d"
...
#    cd ${LSDIR}/$MDIR/b/
#    wget -c -a /tmp/SVBH-downlog ftp://157.157.145.99/$MDIR/b/$d
