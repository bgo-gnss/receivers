...
STATION=OLKE
STATION_FULL_NAME="Ölkelduháls"
station=olke
sss=olke

IPNR=157.157.40.232:8080       		# netr9 web access and programable interface
IPNRFTP=157.157.40.232        		# netr9 conel router
  ...
  then
    ...
    then
       ...
       then
         ...
         echo -e $MESSAGE "\t(" $IPNRFTP ") : Answers ping!"
       fi
     else
       echo -e $MESSAGE "\t(" $IPNRFTP ") : Answers ping!"
     fi
...
VSTR=`curl 'http://'$IPNR'/prog/show?voltages' | grep port`
echo "-- > " $VSTR
...
DIRLIST=`curl 'http://'${IPNR}'/prog/show?directory&path=/Internal/' 2>/dev/null|  grep 'directory name' | grep -v 'lost' | awk '{print substr($2,6,6)}' | grep -v '20Hz_1h' | grep -v '10Hz' | grep -v 'Clone' | tail -${MONTHS}`
...
do
  ...
  curl 'http://'${IPNR}'/prog/show?directory&path=/Internal/'${MDIR}'/15s_24hr'  2>/dev/null | grep name | grep -v .T0B | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
    echo "-- > Getting file $d"
    cd $LSDIR/$MDIR/a
    #wget -c ftp://${IPNR}/Internal/$MDIR/15s_24h/$d
    curl "http://$IPNR/prog/download?file&path=/Internal/$MDIR/15s_24hr/$d" > $d
...
then
  ...
  cd $LSDIR
  curl 'http://'${IPNR}'/prog/show?directory&path=/Internal/'${MDIR}'/1Hz_1hr'  2>/dev/null | grep name | grep -v .T0B | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  ...
  do
    ...
    curl "http://$IPNR/prog/download?file&path=/Internal/$MDIR/1Hz_1hr/$d" > $d
  done            # closes file loop 
 fi
done            # closes month loop
