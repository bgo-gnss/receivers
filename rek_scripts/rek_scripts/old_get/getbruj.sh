#!/bin/sh
# Hourly and daily download of Trimble NETRS receiver BRUJ
#HG
# HG dec 2009: Use dump instead of brot
# HG jun 2010: Use wget download line from Benni Ofeigs. Note that BRUJ does not sample into session sub-directory
#################
STATION=BRUJ
station=bruj
sss=bruj
...
# Variables:
IPNR=157.157.40.2:8080       # netrs ipnumber
IPNRFTP=157.157.40.2       # netrs ipnumber
LSDIR=/data/$STATION
...
do
  ...
  echo "there are `grep BRUJ dlist.tmp | wc -l | awk '{print $1}'` 15 sec files to download from the receiver for month $MDIR"
  DLIST=`cat dlist.tmp`
  for d in $DLIST
  do
    echo "getting file $d"
    ...
    cd $LSDIR/$MDIR
    #wget -c -a /tmp/BRUJ-downlog ftp://$IP/$MDIR/$d
...
#  # get missing b-files:
#  echo "there are `grep BRUJ dlist.tmp | wc -l | awk '{print $1}'` 1 sec files to download from the receiver for month $MDIR"
#  DLIST=`cat dlist.tmp`
#  for d in $DLIST
#  do
#    echo "getting file $d"
...
#    cd ${LSDIR}/$MDIR/b/
#    wget -c -a /tmp/BRUJ-downlog ftp://157.157.145.99/$MDIR/b/$d
