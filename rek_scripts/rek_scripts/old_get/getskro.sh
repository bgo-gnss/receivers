...
# Variables:
STATION_FULL_NAME="Skrokkalda"
STATION=SKRO
station=skro

#IP=194.144.255.177       		# netr9 web access and programable interface
IP=157.157.222.55
HTTP_PORT=':8080'				# HTTP
#FTP_PORT=':2160'				# FTP
...
PNGCNT=`ping -c1 -w1 $IP |grep transmit|awk '{print $4}'`
  ...
  then
    ...
    echo "-- > Will try again.."
    sleep 3
    PNGCNT=`ping -c3 -w3 $IP |grep transmit|awk '{print $4}'`
    ...
    then
       ...
       echo "-- > Will try again.."
       sleep 3
       PNGCNT=`ping -c5 -w5 $IP |grep transmit|awk '{print $4}'`
...
then
  ...
  do
    echo "-- > Getting file $d"
    cd $LSDIR/$MDIR/b
    #wget -c ftp://${IPNR}/Internal/$MDIR/1Hz_1hr/$d
