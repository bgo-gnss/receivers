#!/bin/sh
# Hourly and daily download of Trimble NETRS receiver SKOG
# SFS jun 2011: Adapted from getsoho.sh to download from SKOG which now has 3G conection.
#################
...
STATION=SAUR
STATION_FULL_NAME="Saurbær"
station=saur
sss=saur

IP=157.157.40.149
IPNR=157.157.40.149:8080       		# netrs ipnumber 
IPNRFTP=157.157.40.149:2121     	# netrs ipnumber

DLIM="1.7k"                                # limit the bandwith usage while downloading
...
do
  ...
  do
    ...
    cd ${LSDIR}/$MDIR/b/
    wget -c --limit-rate=${DLIM} ftp://${IPNRFTP}/$MDIR/b/$d
...
ls ${MDIR}/b/*T00 > currlist_met.lst
...
do
  ...
  do
    echo ">> Getting file $d"
    cd $LSDIR/$MDIR/a
    wget -c --limit-rate=${DLIM} ftp://${IPNRFTP}/$MDIR/a/$d
...
# see if files have already been transformed to rinex
  ...
  if [ $DFLAG -eq 0 ]
  then
     echo ">> Files that will be transformed: " $f
     # file has not been transformed
     cp $f $TMPDIR/.
     echo $f >> donelist.lst
   fi
