#!/bin/sh
# Hourly and daily download of Trimble NETRS receive
# SFS jun 2011: Adapted from getsoho.sh to download from GFUM which now has 3G conection.
# SFS jan 2013: Adopted for use with connect.py for ask stations
#################
...
STATION=HAHV
STATION_FULL_NAME="Hafrahvammagljúfur"
station=hahv
sss=hahv

IPNR=localhost:8101      		# netrs ipnumber 
IPNRFTP=localhost:8101    		# netrs ipnumber
IPHOST=10.140.48.23
...
B_FILES=1
MONTHS=2				#use min 2 months so the last file of last month is picked up :)
...
echo "Checking if the receiver is live and kicking:"
ssh sil@kjarni "ping -c1 -w1 ${IPHOST}" > ping1.tmp
PNGCNT=`cat ping1.tmp |grep transmit|awk '{print $4}'`
  if [ "$PNGCNT" -eq 0 ]
  then
    echo "-- > Ping check 1: Does not answer ping..."
    echo "-- > Will try again.."
    sleep 3
    ssh sil@kjarni "ping -c3 -w3 ${IPHOST}" > ping2.tmp
    PNGCNT=`cat ping2.tmp |grep transmit|awk '{print $4}'`
    ...
    then
       ...
       echo "-- > Will try again.."
       sleep 3
       ssh sil@kjarni "ping -c5 -w5 ${IPHOST}" > ping3.tmp
       PNGCNT=`cat ping3.tmp |grep transmit|awk '{print $4}'`
	  ...
	  rm ping3.tmp
         ...
         echo -e $MESSAGE "\t(" $IPHOST ") : Answers ping from kjarni!"
       fi
     else
       echo -e $MESSAGE "\t(" $IPHOST ") : Answers ping from kjarni!"
     fi
...
# Open up a ssh tunnel with python
#python /usr/sil/gps/python2/hahvConn.py
...
DIRLIST=`curl --connect-timeout 5 'http://'${IPNR}'/prog/show?loggedfiles&directory=/' 2>/dev/null|  grep Directory | awk '{print substr($2,6,6)}' | tail -${MONTHS}`
for MDIR in $DIRLIST
do
  cd ${LSDIR}
...
######## get a-file list:  #########
  ...
  curl --connect-timeout 5 'http://'${IPNR}'/prog/show?loggedfiles&directory=/'${MDIR}'/a'  2>/dev/null | grep name | awk '{print substr($2,6,22), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
    ...
    curl --connect-timeout 5 'http://'${IPNR}'/prog/download?loggedfile&path=/'$MDIR'/a/'$d'' > $MDIR/a/$d
...
######## get b-file list:  #########
  ...
  curl --connect-timeout 5 'http://'${IPNR}'/prog/show?loggedfiles&directory=/'${MDIR}'/b'  2>/dev/null | grep name | awk '{print substr($2,6,22), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  pwd
  # check if more than one file NEED TO ADD IF LOOP
  ...
  do
    ...
    curl --connect-timeout 5 --retry 7 'http://'${IPNR}'/prog/download?loggedfile&path=/'$MDIR'/b/'$d'' > $MDIR/b/$d
  done            # closes file loop
fi
done            # closes month loop
...
##### Close a ssh tunnel with python
#python /usr/sil/gps/python2/hahvDisConn.py
#
