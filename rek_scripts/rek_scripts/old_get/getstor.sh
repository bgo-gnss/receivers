...
# Variables:
STATION_FULL_NAME="Stórólfshvoll"
STATION=STOR
station=stor

IP=localhost       		# netr9 web access and programable interface
HTTP_PORT=':8121'			# HTTP
#FTP_PORT=':2160'			# FTP
...
MENUM2=`grep "vi get${station}.sh" /tmp/tmpps.$$ | wc -l`
...
then
  if [ $MENUM2 -gt 0 ]
  then 
   echo "-- > get${station}.sh is being edited with vi.. exiting..."; exit
   echo " "
  else 
...
#python /usr/sil/gps/python2/fteyDisConn.py
#python /usr/sil/gps/python2/fteyDisConn.py
#python /usr/sil/gps/python2/fteyConn.py
...
VSTR=`curl 'http://'$IP$HTTP_PORT'/prog/show?Voltage&input=2'`
echo "-- > " $VSTR
...
DIRLIST=`curl 'http://'$IP$HTTP_PORT'/prog/show?loggedfiles&directory=/' 2>/dev/null|  grep Directory | awk '{print substr($2,6,6)}' | tail -${MONTHS}`
...
do
  ...
  curl 'http://'$IP$HTTP_PORT'/prog/show?loggedfiles&directory=/'${MDIR}'/a'  2>/dev/null | grep name | awk '{print substr($2,6,22), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
    echo "-- > Getting file $d"
    cd $LSDIR/$MDIR/a
    curl "http://$IP$HTTP_PORT/prog/download?loggedfile&path=/'$MDIR'/a/'$d'" > $d
...
then
  ...
  cd $LSDIR
  curl 'http://'$IP$HTTP_PORT'/prog/show?loggedfiles&directory=/'${MDIR}'/b'  2>/dev/null | grep name | awk '{print substr($2,6,22), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  ...
  do
    ...
    curl "http://$IP$HTTP_PORT/prog/download?loggedfile&path=/'$MDIR'/b/'$d'" > $d
  done            # closes file loop 
 fi
done            # closes month loop
...
ls */a/*a.$RAW_FILE_ENDING > currlist.lst
ls */b/*b.$RAW_FILE_ENDING > currlist1.lst
...
echo " "
cd /data/${STATION}/tmp
/home/gpsops/bin/mall_netrs_t00.sh *.$RAW_FILE_ENDING
...
/home/gpsops/bin/mall_netrs_1hz_t00.sh *.$RAW_FILE_ENDING
