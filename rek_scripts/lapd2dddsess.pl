...
# Laga hlaupaarsbogg i GPS vikureikningi [HG feb 2012]
#############################################################################
#use strict # seems obsolete
...
# Calculate GPS WEEK (weeks from 6. jan 1980):
my ($gweek,$idx);
$idx=leap($YEAR);
$gweek=int( (($YEAR-1980)*365+int(($YEAR-1980)/4)+$doy-5)/7 );
if ($idx) {$gweek=int( (($YEAR-1980)*365+int(($YEAR-1980)/4)+$doy-6)/7 );}
