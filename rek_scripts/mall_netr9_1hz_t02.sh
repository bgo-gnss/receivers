...
do
  ...
  # else
   ...
   ${RBINDIR}runpkr00 -v -d -g -s $LFILE     #obsjón f ef vandræði
   ...
   ${TEQCPATH}teqcc  -week ${WEEK} +C2  $DATFIL  > $OBSFIL   # +C2 added for NetR9/T02 data
   ${TEQCPATH}teqcc +err err.lst -config ${CONPATH}/${CONFIL} $OBSFIL > tmp2.$$
   mv tmp2.$$ $OBSFIL
   ${TEQCPATH}teqcc +qc -plot $OBSFIL > $QCFIL
   #/home/gpsops/bin/thjap $OBSFIL

   echo $REALBAS
   
   # move and clean up
   rm ${RELBAS}.tgd ${RELBAS}.ion ${RELBAS}.mes ${RELBAS}.eph 
