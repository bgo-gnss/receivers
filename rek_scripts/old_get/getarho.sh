...
# Variables:
STATION_FULL_NAME="Árholt"
STATION=ARHO
station=arho

IP=157.157.222.109       		# netr9 web access and programable interfac
HTTP_PORT=':8080'				# HTTP
#FTP_PORT=':2160'				# FTP
...
do
  ...
  curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/'${MDIR}'/15s_24'  2>/dev/null | grep name | grep -v .T0B | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
    echo "-- > Getting file $d"
    cd $LSDIR/$MDIR/a
    curl "http://$IP$HTTP_PORT/prog/download?file&path=/Internal/$MDIR/15s_24/$d" > $d
