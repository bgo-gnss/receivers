...
# Variables:
STATION_FULL_NAME="Grímsey"
STATION=GMEY
station=gmey

SESSION_15s=a
SESSION_1s=b
SESSION_15s_FOLDER=15s_24hr
SESSION_1s_FOLDER=1Hz_1hr
SESSION_20hz_FOLDER=20Hz_1hr
...
#python /usr/sil/gps/python2/gmeyConn.py
...
DIRLIST=`curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/' 2>/dev/null|  grep 'directory name' | grep -v 'lost' | awk '{print substr($2,6,6)}' | grep -v '30s_8h' | grep -v Clone | tail -${MONTHS}`
...
do
  ...
  curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/'${MDIR}'/'${SESSION_15s_FOLDER}''  2>/dev/null | grep name | grep -v .T0B | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
    echo "-- > Getting file $d"
    cd $LSDIR/$MDIR/a
    curl "http://$IP$HTTP_PORT/prog/download?file&path=/Internal/$MDIR/$SESSION_15s_FOLDER/$d" > $d
...
then
  ...
  cd $LSDIR
  curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/'${MDIR}'/'${SESSION_1s_FOLDER}''  2>/dev/null | grep name | grep -v .T0B | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  ...
  do
    ...
    curl "http://$IP$HTTP_PORT/prog/download?file&path=/Internal/$MDIR/$SESSION_1s_FOLDER/$d" > $d
  done            # closes file loop 
 fi
done            # closes month loop
...
#### Disconnect the SSH tunnels ####
#python /usr/sil/gps/python2/gmeyDisConn.py
####
...
ls */a/*$SESSION_15s.T02 > currlist.lst
ls */b/*$SESSION_1s.T02 > currlist1.lst
