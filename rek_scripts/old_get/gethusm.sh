...
STATION=HUSM
STATION_FULL_NAME="Husmuli"
station=husm
sss=husm

IPNR=157.157.40.119:8060    		        # netr9 web access and programable interface
IPNRFTP=157.157.40.119:2160         		# netr9 conel router
...
DIRLIST=`curl 'http://'${IPNR}'/prog/show?directory&path=/Internal/' 2>/dev/null|  grep 'directory name' | grep -v 'lost' | awk '{print substr($2,6,6)}' | grep -v '30s_8h' | tail -3`
...
do
  ...
  curl 'http://'${IPNR}'/prog/show?directory&path=/Internal/'${MDIR}'/15s_24hr'  2>/dev/null | grep name | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
    echo "-- > Getting file $d"
    cd $LSDIR/$MDIR/a
    wget -c ftp://${IPNRFTP}/Internal/$MDIR/15s_24hr/$d
...
##### B-files #####
  ...
  cd $LSDIR
  curl 'http://'${IPNR}'/prog/show?directory&path=/Internal/'${MDIR}'/1Hz_1hr'  2>/dev/null | grep name | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  ...
  do
    echo "-- > Getting file $d"
    cd $LSDIR/$MDIR/b
    wget -c ftp://${IPNRFTP}/Internal/$MDIR/1Hz_1hr/$d
  done            # closes file loop
done            # closes month loop
