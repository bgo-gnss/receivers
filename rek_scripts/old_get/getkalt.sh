...
STATION=KALT
STATION_FULL_NAME="Kálfholt í Holtum"
station=kalt
sss=kalt

IPNR=88.149.12.119     		# netrs ipnumber 
IPNRFTP=88.149.12.119  		# netrs ipnumber
...
TSTR=`curl -u ${USER}:${PWS} 'http://'$IPNR'/prog/show?temperature'`
echo ">> "$TSTR
VSTR=`curl -u ${USER}:${PWS} 'http://'$IPNR'/prog/show?Voltage&input=2'`
echo ">> "$VSTR
...
DIRLIST=`curl -u ${USER}:${PWS} 'http://'${IPNR}'/prog/show?loggedfiles&directory=/' 2>/dev/null|  grep Directory | awk '{print substr($2,6,6)}' | tail -2`
for MDIR in $DIRLIST
do
  cd ${LSDIR}
...
######## get a-file list:  #########
  ...
  curl -u ${USER}:${PWS} 'http://'${IPNR}'/prog/show?loggedfiles&directory=/'${MDIR}'/a'  2>/dev/null | grep name | awk '{print substr($2,6,22), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
   ...
   # wget -c ftp://${IPNRFTP}/$MDIR/a/$d
    curl -u ${USER}:${PWS} 'http://'${IPNR}'/prog/download?loggedfile&path=/'$MDIR'/a/'$d'' > $MDIR/a/$d
...
######## get b-file list:  #########
  ...
  curl -u ${USER}:${PWS} 'http://'${IPNR}'/prog/show?loggedfiles&directory=/'${MDIR}'/b'  2>/dev/null | grep name | awk '{print substr($2,6,22), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  ...
  do
    ...
    curl -u ${USER}:${PWS} 'http://'${IPNR}'/prog/download?loggedfile&path=/'$MDIR'/b/'$d'' > $MDIR/b/$d
  done            # closes file loop
done            # closes month loop
