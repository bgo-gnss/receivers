...
# Variables:
STATION=LFEL
STATION_FULL_NAME="LFEL"
station=lfel
sss=lfel

IPNR=157.157.40.185:1563      		# netrs ipnumber 
IPNRFTP=157.157.40.185:1563    		# netrs ipnumber
...
DIRLIST=`curl 'http://'${IPNR}'/prog/show?loggedfiles&directory=/' 2>/dev/null|  grep Directory | awk '{print substr($2,6,6)}' | tail -$MONTH`
for MDIR in $DIRLIST
do
  cd ${LSDIR}
