#!/bin/sh
# Hourly and daily download of Trimble NETR9 receiver HLID
#################
...
# Variables:
STATION_FULL_NAME="Hlíðardalsskóli"
STATION=HLID
station=hlid

IP=157.157.222.166    		       		# netr9 web access and programable interface
HTTP_PORT=':8080'				# HTTP
#FTP_PORT=':2160'				# FTP
...
PNGCNT=`ping -c1 -w2 $IP |grep transmit|awk '{print $4}'`
  if [ $PNGCNT -eq 0 ]
  then
    echo "-- > Ping check 1: Does not answer ping..."
    echo "-- > Will try again.."
    sleep 3
    PNGCNT=`ping -c1 -w4 $IP |grep transmit|awk '{print $4}'`
    ...
    then
       ...
       echo "-- > Will try again.."
       sleep 3
       PNGCNT=`ping -c1 -w8 $IP |grep transmit|awk '{print $4}'`
...
do
  ...
  curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/'${MDIR}'/15s_24hr'  2>/dev/null | grep name | grep -v .T0B | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist.tmp
  ...
  do
    echo "-- > Getting file $d"
    cd $LSDIR/$MDIR/a
    curl "http://$IP$HTTP_PORT/prog/download?file&path=/Internal/$MDIR/15s_24hr/$d" > $d
...
then
  ...
  cd $LSDIR
  curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/'${MDIR}'/1Hz_1hr'  2>/dev/null | grep name | grep -v .T0B | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  ...
  do
    ...
    curl "http://$IP$HTTP_PORT/prog/download?file&path=/Internal/$MDIR/1Hz_1hr/$d" > $d
  done            # closes file loop 
 fi
done            # closes month loop
