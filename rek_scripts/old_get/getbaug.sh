...
# Variables:
STATION_FULL_NAME="Baughóll"
STATION=BAUG
station=baug

IP=157.157.100.21    		       		# netr9 web access and programable interface
HTTP_PORT=':8060'				# HTTP
#FTP_PORT=':2160'				# FTP
...
then
  ...
  cd $LSDIR
  curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/'${MDIR}'/1HZ'  2>/dev/null | grep name | grep -v .T0B | awk '{print substr($2,6,28), substr($3,6,8)}' | grep ${STATION} > tmplist1.tmp
  ...
  do
    ...
    curl "http://$IP$HTTP_PORT/prog/download?file&path=/Internal/$MDIR/1HZ/$d" > $d
  done            # closes file loop 
 fi
done            # closes month loop
