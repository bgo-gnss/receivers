...
# Variables:
STATION_FULL_NAME="Langahlíð"
STATION=LAHL
station=lahl

IP=157.157.112.104     		# netr9 web access and programable interface
HTTP_PORT=':8060'			# HTTP
FTP_PORT=':2160'			# FTP
...
DIRLIST=`curl 'http://'$IP$HTTP_PORT'/prog/show?directory&path=/Internal/' 2>/dev/null|  grep 'directory name' | grep -v 'lost' | grep -v 'Clone'| awk '{print substr($2,6,6)}' | grep -v '30s_8h' | tail -${MONTHS}`
...
sh /home/gpsops/bin/mall_netr9_1hz_t02_bardarbunga.sh *.T02
