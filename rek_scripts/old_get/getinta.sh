...
# (1) Create the paths marked with #CREATE# on rek
# (2) Touch the files marked with #TOUCH# on rek
# (3) Adapt messages as needed, where #ADAPT# is a tag.
# (4) Create the station data folder under /data/{STATION}, e.g. /data/INTA
# 
# This should be it.
...
STATION=INTA
STATION_FULL_NAME="Inntakshús Kárahnjúkavirkjunar"
station=inta
sss=inta

IPNR=localhost:8100      		# netrs ipnumber 
IPNRFTP=localhost:8100    		# netrs ipnumber
IPHOST=10.140.48.20
...
#python /home/gpsops/python2/intaConn.py
...
DIRLIST=`curl 'http://'${IPNR}'/prog/show?loggedfiles&directory=/' 2>/dev/null|  grep Directory | awk '{print substr($2,6,6)}' | tail -${MONTHS}`
for MDIR in $DIRLIST
do
  cd ${LSDIR}
...
#python /home/gpsops/python2/intaDisConn.py
