...
class ArchiveValidatorBenchmarks:
    ...
    def __init__(self):
        ...
        create_test_file(
            self.invalid_file, size=2048, is_compressed=True, gzip_magic=b"\x00\x00"
        )
    ...
    def bench_find_existing_archive(self):
        ...
        def operation():
            self.validator.find_existing_archive(
                "valid.sbf", str(self.valid_file.with_suffix("")), self.temp_dir
            )
    ...
    def bench_batch_validation(self):
        ...
        archive_dict = {
            f"file{i}.sbf": str(self.temp_dir / f"file{i}.sbf.gz") for i in range(100)
        }

        def operation():
            self.validator.batch_validate_archives(
                files_dict, archive_dict, self.temp_dir
            )
...
class TimeProcessorBenchmarks:
    ...
    def __init__(self):
        ...
        self.test_datetime_list = [datetime(2025, 9, 24, i, 30, 0) for i in range(24)]
    ...
    def bench_immediate_mode_single_file(self):
        ...
        def operation():
            ...
            archiver.archive_file(
                file_path, archive_path, compress=True, remove_tmp=False
            )
