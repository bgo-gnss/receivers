...
from receivers.utils.archive_validator import ArchiveValidator, ArchiveLocation
...
class ReceiverWorkflowSimulator:
    ...
    def step1_generate_file_list(self) -> Dict[str, str]:
        ...
        if HAS_GTIMES:
            ...
            dt_list = gt.datepathlist("#datelist", freq, self.start, self.end, closed="both")
        else:
            # Simplified datetime generation
            dt_list = []
            current = self.start
            increment = timedelta(hours=1) if "1hr" in self.session else timedelta(days=1)
        ...
        for dt in normalized_list:
            if self.receiver_type == "polarx5":
                ...
                filename = f"{self.station_id}{dt.strftime('%Y%m%d%H%M')}{session_letter}.sbf"
                remote_path = f"/DSK1/SSN/{self.session}/{dt.strftime('%Y')}/{dt.strftime('%V')}"
            elif self.receiver_type == "netr9":
                # NetR9: STATION%Y%m%d%H%Mb.T02
                session_letter = 'b' if "1hr" in self.session else 'a'
                filename = f"{self.station_id}{dt.strftime('%Y%m%d%H%M')}{session_letter}.T02"
        ...
        self.logger.info(f"Example: {list(files_dict.keys())[0]} -> {list(files_dict.values())[0]}")

        return files_dict

    def step2_create_existing_archives(self, files_dict: Dict[str, str], percentage: int = 50):
        ...
        files_to_create = list(files_dict.keys())[:int(len(files_dict) * percentage / 100)]

        self.logger.info(f"Creating {len(files_to_create)} existing archive files ({percentage}%)")
        ...
        self.logger.info(f"Created {len(files_to_create)} archive files in {self.archive_dir}")

    def step3_validate_archives(
        self,
    ...
    def step4_simulate_download(self, missing_files: Dict[str, str]) -> List[Path]:
        ...
        self.logger.info(f"Simulating download of {len(missing_files)} files to {self.tmp_dir}")
    ...
    def step5a_archive_immediate_mode(self, downloaded_files: List[Path]):
        ...
        with FileArchiver(logger=self.logger, mode=ArchiveMode.IMMEDIATE) as archiver:
            ...
            self.logger.info(f"Archived: {stats['successful']}/{stats['total_files']} files")
            self.logger.info(f"Average compression: {stats['average_compression_ratio']:.1f}%")
    ...
    def step6_verify_final_state(self, files_dict: Dict[str, str]):
        ...
        self.logger.info(f"Archive verification:")
