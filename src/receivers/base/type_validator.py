...
class ReceiverTypeValidator:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self.http_signatures = {
            "PolaRX5": {
                "endpoints": ["/"],
                "response_indicators": ["septentrio", "polarx", "sbf", "web interface"],
                "headers": ["server"],
                "ports": [80, 8080, 443],
            },
            "NetR9": {
            ...
            "G10": {
                "endpoints": ["/"],
                "response_indicators": ["leica", "gnss", "system"],
                "headers": ["server"],
                "ports": [80, 8080, 443],
            },
        }

    def validate_station_receiver_type(
        ...
        result = {
            ...
            "confidence_scores": {
                dt: self._calculate_confidence(dt, detected_types)
        ...
        if not match_found:
            best_match = max(
                detected_types, key=lambda x: result["confidence_scores"][x]
            )
            result["suggestion"] = {
            ...
            self.logger.info(
                f"✅ Receiver type validation passed for {station_id}: {configured_type}"
            )
    ...
    def _probe_receiver(self, ip: str, station_id: str) -> List[str]:
        ...
        for receiver_type, signature in self.http_signatures.items():
            confidence = self._test_receiver_signature(
                ip, station_id, receiver_type, signature
            )
    ...
    def _test_receiver_signature(
        self, ip: str, station_id: str, receiver_type: str, signature: Dict[str, Any]
    ) -> float:
        ...
        for port in signature["ports"]:
            try:
                ...
                for endpoint in signature["endpoints"]:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            content_score = self._score_content_match(
                                content, signature["response_indicators"]
                            )
                            if content_score > 0:
                            ...
                            header_score = self._score_header_match(
                                headers, signature.get("headers", [])
                            )
                            if header_score > 0:
                                confidence_factors.append(
                                    header_score * 0.5
                                )  # Headers less reliable

                        # Special case: Trimble receivers respond with specific HTTP structure
                        elif response.status_code == 404 and receiver_type in [
                            "NetR9",
                            "NetRS",
                        ]:
                            if "/prog/show?" in endpoint:
                                ...
                                self.logger.debug(
                                    f"{station_id}: {receiver_type} partial match on {url} (404)"
                                )
                ...
                self.logger.debug(
                    f"{station_id}: Error testing {receiver_type} on {ip}:{port}: {e}"
                )
                continue
    ...
    def _score_header_match(
        self, headers: Dict[str, str], header_keys: List[str]
    ) -> float:
    ...
    def _calculate_confidence(
        self, receiver_type: str, detected_types: List[str]
    ) -> float:
    ...
    def batch_validate_stations(
        self, station_configs: Dict[str, Dict[str, Any]]
        ...
        self.logger.info(
            f"Starting batch validation of {len(station_configs)} stations"
        )
        ...
        for station_id, config in station_configs.items():
            try:
                ...
                if result["validation_status"] == "mismatch":
                    ...
                    self.logger.info(
                        f"✅ {station_id}: {result['configured_type']} verified"
                    )
    ...
    def generate_correction_report(
        ...
        report = "🔧 RECEIVER TYPE CORRECTION REPORT\n"
        ...
        report += (
            "To apply corrections, update the corresponding entries in stations.cfg\n"
        )
