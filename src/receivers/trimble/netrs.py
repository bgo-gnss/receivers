...
class NetRS(BaseReceiver):
    ...
    def _process_time_parameters(
        ...
        # Respect the CLI-provided time range (like NetR9 does)
    ...
    def _archive_files(
        ...
        for file_path in downloaded_files:
            try:
                ...
                if filename in archive_files_dict:
                    # Get file size before archiving (matching NetR9 pattern)
                    ...
                    # Check if archive file already exists (matching NetR9 pattern)
                    ...
                    # NetRS files are always raw .T00 - compress them during archiving
                    ...
                    # Verify compressed archive (matching NetR9 pattern)
                    if archive_path.exists():
        ...
        # Log final archiving results (matching NetR9 pattern)
        if archived_count > 0:
