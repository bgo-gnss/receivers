...
class TimeSeriesHealthExtractor:
    ...
    def extract_daily_health(
        ...
        """
        self.logger.info(f"Extracting daily health for {self.station_id} on {date.date()}")
        ...
        for sbf_file in sbf_files:
            try:
                ...
                self.logger.debug(f"Extracted {len(samples)} samples from {sbf_file.name}")
    ...
    def _extract_file_samples(self, sbf_file: Path) -> Tuple[List[Dict], Dict]:
        ...
        for record in channel_data:
            ...
            if dt:
                ...
                by_system = {}
                for system in ['GPS', 'GLONASS', 'Galileo', 'BeiDou', 'QZSS', 'IRNSS', 'SBAS']:
        ...
        if sat_totals:
            stats['satellites'] = {
                'total': {
                    ...
                    'std': round(statistics.stdev(sat_totals), 1) if len(sat_totals) > 1 else 0.0,
                    'min': min(sat_totals),
                    'max': max(sat_totals),
            ...
            for system in ['GPS', 'GLONASS', 'Galileo', 'BeiDou', 'QZSS', 'IRNSS', 'SBAS']:
                ...
                if system_counts and any(c > 0 for c in system_counts):
                    by_system_stats[system] = {
                        ...
                        'std': round(statistics.stdev(system_counts), 1) if len(system_counts) > 1 else 0.0,
    ...
    def _calculate_hourly_stats(
        ...
        for sample in samples:
            try:
                timestamp = datetime.fromisoformat(sample['time'].replace('Z', '+00:00'))
                hour = timestamp.hour
                samples_by_hour[hour].append(sample)
            except Exception as e:
                self.logger.warning(f"Invalid timestamp in sample: {sample.get('time')}: {e}")
    ...
    def _build_metadata(
        ...
        completeness = (len(samples) / expected_samples * 100) if expected_samples > 0 else 0.0
        ...
        if len(samples) > 1:
            for i in range(1, len(samples)):
                try:
                    t1 = datetime.fromisoformat(samples[i-1]['time'].replace('Z', '+00:00'))
                    t2 = datetime.fromisoformat(samples[i]['time'].replace('Z', '+00:00'))
