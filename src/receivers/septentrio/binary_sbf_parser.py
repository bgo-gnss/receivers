...
class BinarySBFParser:
    ...
    def _parse_sbf_message(self, data: bytes, offset: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            message_name = self.HEALTH_MESSAGE_IDS.get(
                message_id, f"Unknown_{message_id:04X}"
            )

            # Different message types have different lengths in filtered files
            if message_name == "PowerStatus":
                message_length = (
                    16  # PowerStatus: bytes 4-5 contain voltage, not length
                )
            ...
            if offset + message_length > len(data):
                if len(self.parsed_messages) < 3:
                    print(
                        f"    Insufficient data for {message_name}: need {message_length} bytes"
                    )
                return None
            ...
            if len(self.parsed_messages) < 3:
                print(
                    f"    Message ID: {message_id} ({message_id:04X}), Name: {message_name}"
                )
            ...
            if message_name == "PowerStatus":
                message.update(
                    self._parse_power_status_full(
                        data[offset : offset + message_length]
                    )
                )
            elif message_name == "DiskStatus":
                message.update(
                    self._parse_disk_status_full(data[offset : offset + message_length])
                )
    ...
    def _parse_power_status(self, payload: bytes) -> Dict[str, Any]:
        ...
        try:
            if len(payload) >= 4:
                ...
                status_flags = (
                    struct.unpack(">H", payload[2:4])[0] if len(payload) >= 4 else 0
                )
        ...
        try:
            if len(full_message) >= 52:
                ...
                for i in range(12, min(48, len(full_message) - 4), 4):
                    ...
                    if (
                        1000000 < value < 20000000000
                    ):  # Reasonable disk space range (1MB - 20GB)
                ...
                return {
                    ...
                    "total_space_candidates": total_space_candidates[
                        :3
                    ],  # Top 3 candidates
    ...
    def _parse_disk_status(self, payload: bytes) -> Dict[str, Any]:
        ...
        try:
            if len(payload) >= 16:
                ...
                total_space = (
                    struct.unpack("<I", payload[4:8])[0] if len(payload) >= 8 else 0
                )
                free_space = (
                    struct.unpack("<I", payload[8:12])[0] if len(payload) >= 12 else 0
                )

                return {
                    "total_disk_kb": total_space,
                    "free_disk_kb": free_space,
                    "used_disk_kb": total_space - free_space
    ...
    def _parse_receiver_time(self, payload: bytes) -> Dict[str, Any]:
        ...
        try:
            if len(payload) >= 8:
                ...
                utc_offset = (
                    struct.unpack("<h", payload[4:6])[0] if len(payload) >= 6 else 0
                )
                time_flags = (
    ...
    if os.path.exists(power_status_file):
        ...
        if messages:
            ...
            for i, msg in enumerate(messages[:3]):  # Show first 3
                ...
                if "voltage_v" in msg:
                    print(
                        f"  Voltage: {msg['voltage_v']:.2f}V (from {msg.get('voltage_field_location', 'unknown field')})"
                    )
        ...
        if messages:
            ...
            for i, msg in enumerate(messages[:3]):  # Show first 3
                ...
                if "total_space_candidates" in msg:
                    print(
                        f"  Message length: {msg.get('message_length_bytes', 0)} bytes"
                    )
