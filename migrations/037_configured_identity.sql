-- Migration 037: Add configured serial/firmware for full identity mismatch detection
--
-- Adds configured_serial and configured_firmware to stations table.
-- These are populated by the scheduler's config-sync from stations.cfg fields:
--   receiver_serial          → configured_serial
--   receiver_firmware_version → configured_firmware
--
-- station_dashboard_data view is rebuilt to expose these columns so the
-- Grafana station detail panel can compare detected vs configured identity.

BEGIN;

ALTER TABLE stations ADD COLUMN IF NOT EXISTS configured_serial  VARCHAR(30);
ALTER TABLE stations ADD COLUMN IF NOT EXISTS configured_firmware VARCHAR(30);

COMMENT ON COLUMN stations.configured_serial IS
  'Expected receiver serial number from stations.cfg (receiver_serial). '
  'NULL = not configured. Compared against serial_number from live health checks.';

COMMENT ON COLUMN stations.configured_firmware IS
  'Expected firmware version from stations.cfg (receiver_firmware_version). '
  'NULL = not configured. Compared against firmware_version from live health checks.';

-- Rebuild view to expose new columns (only the SELECT list changes)
...
SELECT
    ...
    END AS cpu_status,
    CASE
        WHEN COALESCE(m.satellites_used, m.satellites_tracked) IS NULL THEN 'unknown'
    ...
    s.station_status,
    s.health_check,
    -- Receiver identity (live from health checks)
    s.serial_number,
    s.firmware_version,
    s.detected_model,
    ...
    s.model_mismatch,
    -- Configured identity (from stations.cfg, synced by scheduler)
    s.configured_serial,
...
VALUES ('037_configured_identity')
