-- Migration 034: Fix health_good CTE NULL condition for Trimble receivers
--
-- Problem: health_good CTE in station_dashboard_data uses boolean expressions:
--     AND (NOT dp.control_open_db OR h.control_open)
--     AND (NOT dp.ftp_open_db     OR h.ftp_open)
--     AND (NOT dp.http_open_db    OR h.http_open)
--
-- For Trimble/NetR* receivers, FTP and control ports are not checked, so
-- ftp_open and control_open are always NULL in block_health_summary.
-- BOOL_OR(NULL) = NULL, and NOT NULL = NULL, so NULL OR NULL = NULL,
-- which is falsy in a WHERE clause.  Result: health_good returns no rows
-- for any Trimble station, so debouncing is silently disabled for them
-- and every transient critical reading reaches the dashboard immediately.
--
-- Fix: Use IS NOT TRUE / IS TRUE which treats NULL correctly:
--     NULL IS NOT TRUE  → TRUE  (N/A port, no constraint)
--     FALSE IS NOT TRUE → TRUE  → then requires h.X IS TRUE  (PolaRX5: unchanged)
--     TRUE IS NOT TRUE  → FALSE → condition always passes     (PolaRX5: unchanged)
--
-- No dependent views on station_dashboard_data (icinga_check_data and
-- station_status_summary reference station_latest_metrics directly).
...
VALUES ('034_trimble_debounce_fix')
