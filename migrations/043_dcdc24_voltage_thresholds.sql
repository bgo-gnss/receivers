-- Migration 043: Update dcdc24 voltage thresholds
--
-- SKHA (Skarðshamrar) runs on a 24V DC/DC supply with a long cable, so the
-- receiver sees ~17-18 V (not the 20+ V assumed by the old dcdc24 thresholds).
--
-- New thresholds (matching database.cfg [voltage_dcdc24] and Grafana gauge):
--   critical : < 15.0 V  or > 30.0 V
--   warning  : < 17.0 V  or > 26.5 V
--   ok       : 17.0 – 26.5 V
--
-- Also updates the voltage_status computed column and the overall_status
-- voltage-correction guard in station_dashboard_data.
 ...
 SELECT m.station_id,
    ...
    sc.reachability_source,
        CASE
            ...
            WHEN s.power_type::text = 'dcdc24'::text AND m.voltage >= 15.0::double precision AND m.voltage <= 30.0::double precision THEN
            CASE
                WHEN lh.status_details = 'Voltage'::text AND m.voltage >= 17.0::double precision AND m.voltage <= 26.5::double precision THEN 'healthy'::text
                ELSE 'warning'::text
        ...
        CASE
            WHEN lh.status_details = 'Voltage'::text AND lh.overall_status::text = 'critical'::text AND m.voltage IS NOT NULL AND (s.power_type::text = 'dcdc24'::text AND m.voltage >= 17.0::double precision AND m.voltage <= 26.5::double precision OR s.power_type::text = 'mains'::text AND m.voltage >= 18.0::double precision AND m.voltage <= 28.0::double precision OR s.power_type::text = 'dcdc'::text AND m.voltage >= 12.0::double precision AND m.voltage <= 16.5::double precision OR COALESCE(s.power_type, 'battery'::character varying)::text = 'battery'::text AND m.voltage >= 11.8::double precision AND m.voltage <= 15.0::double precision) THEN NULL::text
    ...
    s.stall_timeout_override,
        ...
        CASE
            ...
            CASE
                ...
                WHEN m.voltage < 17.0::double precision OR m.voltage > 26.5::double precision THEN 'warning'::text
                ELSE 'ok'::text
            END
