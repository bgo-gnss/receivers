-- Migration 035: Fix satellite_status in station_dashboard_data for 0-sat receivers
--
-- Problem: satellite_status CASE only checks satellites_used (from GPS PVT block).
-- Trimble receivers with 0 tracked satellites report satellites_tracked=0 but
-- satellites_used=NULL (no PVT fix possible → no nr_sv value). Result: the view
-- returns 'unknown' instead of 'critical', masking genuine no-signal conditions.
--
-- Migration 033 fixed station_status_summary with the same COALESCE approach.
-- Migration 034 rewrote station_dashboard_data for the Trimble debounce fix but
-- didn't carry the 033 fix across, leaving the bug in the dashboard view.
--
-- Fix: COALESCE(m.satellites_used, m.satellites_tracked) — fall back to raw
-- tracking count when PVT satellites_used is NULL. Both columns are integer types
-- (smallint / bigint) so no cast is needed.
--
-- Affected stations confirmed: GSIG, HEDI (satellites_tracked=0, satellite_status
-- showing 'unknown' in Grafana instead of 'critical').
...
VALUES ('035_satellite_status_dashboard_fix')
